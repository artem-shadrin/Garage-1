...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const trigger = document
    .querySelectorAll(".camera-card__video")
    .forEach((item) =>
  ...
  const activeModal = () => {
    const trigger = (e) => {
      if (
        e.target.classList.contains("modal_player") ||
...
function burgerMenu(icon) {
  icon.parentNode.classList.toggle("menu__burger--active");
  icon.classList.toggle("change");
}
document.addEventListener("DOMContentLoaded", () => {
  const modal = document.querySelector(".modal_create");
  const trigger = document
    .querySelectorAll(".camera-card__title")
    .forEach((item) =>
      item.addEventListener("click", (e) => {
  ...
  const activeModal = () => {
    const trigger = (e) => {
      if (
        e.target.classList.contains("modal_create") ||
        e.target.closest(".close, .create-ad-btn")
      ) {
        modal.classList.remove("modal--show");
        modal.removeEventListener("click", trigger);
      }
